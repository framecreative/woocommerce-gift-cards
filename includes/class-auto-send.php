...
class WCGC_Auto_Send
{
	...
	public function __construct()
	{
		...
		if ( is_admin() )
		{
			add_action( 'get_giftcard_settings', array( $this, 'autosend_page' ), 10, 2 );
			add_filter( 'woocommerce_add_section_giftcard', array( $this, 'autosend_settings' ) );
			add_filter( 'get_giftcard_settings', array( $this, 'auto_send_settings' ), 10, 2 );
			add_filter( 'woocommerce_giftcard_settings', array( $this, 'auto_send_settings' ) );
		}

		add_action( 'woocommerce_checkout_order_processed', array( $this, 'create_automatically' ), 10, 1 );
		add_action( 'woocommerce_order_status_processing', array( $this, 'send_automatically' ), 10, 1 );
		add_action( 'woocommerce_order_status_completed', array( $this, 'send_automatically' ), 10, 1 );
	}
	...
	public function auto_send_settings( $options, $current_section = false )
	{
		if ( $current_section == 'auto' )
		{
			...
			$options = array(
				...
				array( 'type' => 'sectionend', 'id' => 'wcgc_auto_settings' ),
			); // End pages settings
